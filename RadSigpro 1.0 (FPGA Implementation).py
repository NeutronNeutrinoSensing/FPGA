        ...
        for counter,line in enumerate(csv_reader):
            if counter > 0:
                ...
                '''if int(min(line[4:])) <= 2000:
                    ...
                    raw_pulse[:] = [float(p) for p in line[4:]]
                ...
                #pulses_corrected[counter-1] = [(-float(p)+baseline)*0.1220703125 for p in line[4:]]
                
                #Baseline Freeze at Pulse Start Method
                pulses_corrected_first[:] = [(-float(p)) for p in line[4:len(line)]]
                for i,height in enumerate(pulses_corrected_first):
                    if height >= max(pulses_corrected_first):
                        pulses_corrected[counter-1] = [float(p-float(np.average(pulses_corrected_first[i-9:i-4])))*0.1220703125 for p in pulses_corrected_first]
                        
                        '''if counter == 3 or counter == 5:
                            print(f'Baseline Average: {int(np.average(pulses_corrected_first[i-9:i-4]))}, Values Averaged: {pulses_corrected_first[i-9:i-4]}, Maximum Index: {i}')'''
                ...
                #print(f'Pulse {counter-1} Length: {len(line[4:-cfd_delay])}')
                inverted_delayed_signal[(cfd_delay//2):] = [(-float(p)+baseline) for p in line[4:-cfd_delay//2]]
                #inverted_delayed_signal[(cfd_delay//2):] = inverted_delayed_signal_first[cfd_delay::2]
                
                attenuated_signal[:] = [attenuation_fraction*(float(p)-baseline) for p in line[4:len(line)]]
                ...
                times_of_pulses[counter-1] = time_list     #Creates array of time correspondiong to each pulse data point, starting from timetag bin location, and increasing/decreasing by 2ns around it.
...
## This function makes a pulse shape discrimination plot of both the gamma and neutron corrected data on the same graph, with selected max and min pulse heights (pulse start and end shouldn't usually be changed).
    ...
    for i in range(math.ceil(num_total_pulses/pulses_per_csv)):
        if path.exists(f'{gamma_file[:-5]}{i+1}.csv') == True:
            ...
            time_data1[num_pulses_inserted1:(num_pulses + num_pulses_inserted1)] = data[[f'time {j+1} [ns]' for j in range(num_pulses_inserted1, num_pulses + num_pulses_inserted1)]].to_numpy().T
            pulse_data1[num_pulses_inserted1:(num_pulses + num_pulses_inserted1)] = data[[f'pulse {j+1} [mV]' for j in range(num_pulses_inserted1, num_pulses + num_pulses_inserted1)]].to_numpy().T        
    ...
    for i in range(math.ceil(num_total_pulses/pulses_per_csv)):
        if path.exists(f'{neutron_file[:-5]}{i+1}.csv') == True:
            ...
            num_pulses = len(data.columns)//2
            time_data2[num_pulses_inserted2:(num_pulses + num_pulses_inserted2)] = data[[f'time {j+1} [ns]' for j in range(num_pulses_inserted2, num_pulses + num_pulses_inserted2)]].to_numpy().T
            pulse_data2[num_pulses_inserted2:(num_pulses + num_pulses_inserted2)] = data[[f'pulse {j+1} [mV]' for j in range(num_pulses_inserted2, num_pulses + num_pulses_inserted2)]].to_numpy().T        
    ...
    for counter, (pulse1,times1,pulse2,times2) in enumerate(zip(pulse_data1,time_data1,pulse_data2,time_data2)):
        ...
        for i in pulse1[pulse1_max_index - 3 : pulse1_max_index + 155]:
            if i >= 0:
                total_integrals1[counter] += i
        for i in pulse1[pulse1_max_index + 10 : pulse1_max_index + 155]:
            if i >= 0:
                tail_integrals1[counter] += i
        index_of_discrimination1.append(counter)
        for i in pulse2[pulse2_max_index - 3 : pulse2_max_index + 155]:
            if i >= 0:
                total_integrals2[counter] += i
        for i in pulse2[pulse2_max_index + 10 : pulse2_max_index + 155]:
            if i >= 0:
                tail_integrals2[counter] += i
    ...
    if path.exists(f'{neutron_file[:-5]}{1}.csv'):
        plt.hist(gamma_tail_to_total, bins = 200, range = (0,0.6), label = 'Gamma Detector', color='blue', histtype='step')        #Plots histogram of tail to total ratio for each pulse.
        plt.hist(neutron_tail_to_total, bins = 200, range = (0,0.6), alpha=0.6, label = 'Neutron Detector', color='orange', histtype='step')
    ...
    if path.exists(f'{gamma_file[:-21]}_PSD_Data{1}.csv'):
        ...
        for i in range(math.ceil(len(index_of_discrimination1)/50000)):
            ...
            f.write(",".join([f"time {index_of_discrimination1[k]+1} [ns],pulse {index_of_discrimination1[k]+1} [mV],tail/total {index_of_discrimination1[k]+1}" for k in range(start_pulse, final_pulse)])+ "\n")
            for j in range(sample_length): 
    ...
    if path.exists(f'{neutron_file[:-21]}_PSD_Data{1}.csv'):
        ...
        if path.exists(f'{neutron_file[:-5]}{1}.csv'):
            for i in range(math.ceil(len(index_of_discrimination2)/50000)):
                ...
                f.write(",".join([f"time {index_of_discrimination2[k]+1} [ns],pulse {index_of_discrimination2[k]+1} [mV],tail/total {index_of_discrimination2[k]+1}" for k in range(start_pulse, final_pulse)])+ "\n")
                for j in range(sample_length): 
...
## This function makes a pulse heigh distribution plot of a raw data file (not a file already run through the pulse correction function)
    ...
    #print(f'Average Pulse Height: {np.mean(pulse_heights_mV)}')
    '''y1, x1, _ = plt.hist(pulse_heights_mV, bins = int(adc_axis_max*0.1220703125//11), range = (0, int(adc_axis_max*0.1220703125)))
    print(f'Most Common PHD: {x1[np.where(y1 == y1.max())][0]}, Number of Pulses: {y1.max()}')'''
    
    '''spectrum_data = pd.read_csv('BaselineHeld_G8139_and_N8127_PSDCut_CompassSpectrums.csv')
    total_pulses_energy = []
    #total_pulses_energy1 = []
    #total_pulses_energy2 = []
    
    energy_channels = np.arange(0, 16384, 1, dtype=np.float64)
    energy_data = spectrum_data['Neutron Energy'].to_numpy()
    ...
    plt.show()
    plt.hist(pulse_heights_mV, bins = int(adc_axis_max*0.1220703125//11), range = (0, int(adc_axis_max*0.1220703125)))
...
## This function makes a time of flight plot of the neutron times - gamma times, where you select the minimum and maximum points on the x-axis.
    ...
    if path.exists(f'{gamma_file[:-21]}_PSD_Data{1}.csv') and path.exists(f'{neutron_file[:-21]}_PSD_Data{1}.csv'):
        for i in range(math.ceil(num_discriminated_gammas/50000)):           
            if path.exists(f'{gamma_file[:-21]}_PSD_Data{i+1}.csv'):
                ...
                gamma_time_data[num_pulses_inserted3:(num_pulses + num_pulses_inserted3)] = gamma_data[[f'time {j+1} [ns]' for j in range(num_pulses_inserted3, num_pulses + num_pulses_inserted3)]].to_numpy().T
                gamma_pulse_data[num_pulses_inserted3:(num_pulses + num_pulses_inserted3)] = gamma_data[[f'pulse {j+1} [mV]' for j in range(num_pulses_inserted3, num_pulses + num_pulses_inserted3)]].to_numpy().T 
    ...
    if path.exists(f'{gamma_file[:-21]}_PSD_Data{1}.csv') and path.exists(f'{neutron_file[:-21]}_PSD_Data{1}.csv'):
        for i in range(math.ceil(num_discriminated_neutrons/50000)):
            if path.exists(f'{neutron_file[:-21]}_PSD_Data{i+1}.csv'):
                ...
                neutron_time_data[num_pulses_inserted4:(num_pulses + num_pulses_inserted4)] = neutron_data[[f'time {j+1} [ns]' for j in range(num_pulses_inserted4, num_pulses + num_pulses_inserted4)]].to_numpy().T
                neutron_pulse_data[num_pulses_inserted4:(num_pulses + num_pulses_inserted4)] = neutron_data[[f'pulse {j+1} [mV]' for j in range(num_pulses_inserted4, num_pulses + num_pulses_inserted4)]].to_numpy().T 
    ...
    '''y1, x1, _ = plt.hist(times_of_flight, bins= int((abs(time_axis_min)+abs(time_axis_max))*2), range = (int(time_axis_min), int(time_axis_max)))
    ...
    y, bin_edges = np.histogram(tof_cut_gamma_psd, bins= 200, range = (0, 0.6))
    bin_centers = 0.5*(bin_edges[1:] + bin_edges[:-1])
    plt.errorbar(bin_centers, y, yerr = y**0.5, capsize = 2, marker = '.', drawstyle = 'steps-mid', ecolor = 'dimgrey', elinewidth = 1.5, linewidth = 1.5, markeredgewidth = 0, markersize = 0.1, label = 'Gamma Detector', color='blue')        
    ...
    #Plot with error bars
    y, bin_edges = np.histogram(times_of_flight, bins= int((abs(time_axis_min)+abs(time_axis_max))*2), range = (int(time_axis_min), int(time_axis_max)))
    bin_centers = 0.5*(bin_edges[1:] + bin_edges[:-1])
    plt.errorbar(bin_centers, y, yerr = y**0.5, capsize = 2, marker = '.', drawstyle = 'steps-mid', ecolor = 'dimgrey', elinewidth = 1.5, linewidth = 1.5, markeredgewidth = 0, markersize = 0.1, color = 'blue')
    ...
    if path.exists(f'{gamma_file[:-21]}_PSD_Data{1}.csv') and path.exists(f'{neutron_file[:-21]}_PSD_Data{1}.csv'):
        f = open(f'{gamma_file[6:-21]}_ToF_and_PSD.csv', 'w')
...
def ToF_Comparison(gamma_file, neutron_file, show_times, max_time_difference, min_time_difference = 2, max_height_cutoff = 10000, min_height_cutoff = 200, long_gate = 360, pregate = 50, short_gate = 70, cfd_delay = 6, attenuation_fraction = 0.25):
## This function compares gamma and neutron pulse pairs, then plots and records the pairs that fit the specified constraints.
    ...
    f.write(",".join([f"time {k} [ns],pulse {k} [mV]" for k in index_of_discrimination])+ "\n")
    for j in range(sample_length): 
...
#PSD_Analysis('Gamma_ToF11_BaselineHeld_G8138_and_N8125_PSDCut_CorrectedPulses1.csv','Neutron_ToF11_BaselineHeld_G8138_and_N8125_PSDCut_CorrectedPulses1.csv')
#PHD_Plot('Gamma_ToF11_BaselineHeld_G8138_and_N8125_PSDCut.csv', 8000, 8138)
#PHD_Plot('Neutron_ToF11_BaselineHeld_G8138_and_N8125_PSDCut.csv', 8000, 8125)
ToF_Analysis('Gamma_ToF11_BaselineHeld_G8138_and_N8125_PSDCut_CorrectedPulses1.csv', 'Neutron_ToF11_BaselineHeld_G8138_and_N8125_PSDCut_CorrectedPulses1.csv', -35, 40)
